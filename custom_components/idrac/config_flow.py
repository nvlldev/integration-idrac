...
async def validate_input(hass: HomeAssistant, data: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        def test_connection():
            session = Session(hostname=host, community=community, version=2, timeout=10, retries=2)
            test_oid = "1.3.6.1.4.1.674.10892.5.4.600.30.1.6.1.3"
            result = session.get(test_oid)
            return result.value
        ...
        discovered_fans = await _discover_sensors(hass, host, community, port, SNMP_WALK_OIDS["fans"])
        discovered_cpus = await _discover_cpu_sensors(hass, host, community, port, SNMP_WALK_OIDS["cpu_temps"])
...
async def _discover_sensors(
    ...
    try:
        def snmp_walk():
            ...
            try:
                walk_results = session.walk(base_oid)
                for item in walk_results:
                    if item.oid.startswith(base_oid + "."):
                        try:
                            sensor_id = int(item.oid.split(".")[-1])
                            if item.value is not None and str(item.value) != "No Such Object currently exists at this OID":
...
async def _discover_cpu_sensors(
    ...
    all_temp_sensors = await _discover_sensors(hass, host, community, port, base_oid)
